...
function refreshmail(r) {
    if ((/in/.test(document.readyState)) || (undefined === Gmail)) {
        setTimeout('refreshmail(' + r + ')', 10);
    } else {
        r();
    }
}
...
ga('create', 'UA-62757749-1', 'auto');
ga('send', 'pageview');

var main = function () {
    console.log("jquery inside script",typeof $)
    gmail = new Gmail();
    ...
    gmail.observe.on('view_thread', function (obj) {
        gmail.observe.on('view_email', function (obj) {
            ...
            var data = gmail.get.email_data(emailid);
            var threads = data.threads;
            for (var key in threads)
            {
                if (emailid == key)
                {
                    var em_html = threads[key].content_html;
                    var parsed_html = str_dom(em_html);
                    var divs = $(parsed_html).find('div');
                    //console.log(divs.length);   
                    if (divs.length > 0)
                    {
                        var rel_divs = divs[2];
                        var rel_val = $(rel_divs).attr('rel');
                        ...
                        {
                            var dataTo = new Array();
                            z = 0;
                            var recId = new gmail.dom.email(emailid);
                            var to = recId.to();
                            var from = recId.from();
                            ...
                            var fromdata = from.email;
                            var onSave = function (error, result) {
                                if (error) {
                                    console.error("Failed to save details: " + error);
                                } else {
                                    console.log("Details save successfully: ", result)
                                }
                            }
                            Savedetails(rel_val, dataTo, fromdata, onSave);
                            var onCheck = function (error, result) {
                                if (error)
                                {
                                    console.error();
                                } else {
                                    ...
                                    {
                                        $("#dmail").remove();
                                        if (gmail.get.user_email() === fromdata)
                                        {
                                            $("#dmail").remove();
                                            var access = $(".adC.nH").prepend(result.html);
                                            
                                        }
                                        var onValue = function (error, result) {
                                                if (error) {
                                                    console.error("Failed to save details: " + error);
                                                } else {
                                                    console.log("Details save successfully: ", result);
                                                    var hash = result.hash;
                                                    var dec = GibberishAES.dec($(divs[1]).text(), hash);
                                                    var email = new gmail.dom.email(emailid);
                                                    email.body(dec);
                                                }
                                            }
                                            retrieveData(rel_val, onValue);
                                    }
                                    else {
                                        var show = "Sorry your are not authorize to see this mail";
                                        var email = new gmail.dom.email(emailid);
                                        email.body(show);
                                    }
                                    $("input[type=checkbox]").on("click", function () {
                                        var id = $("#emailid").val();
                                        var email = $(this).val();
                                        var onRevoke = function (error, result)
                                        {
                                            if (error) {
                                                console.error("Failed to save details: " + error);
                                            } else {
                                        ...
                                        Revoke(id, email, onRevoke);
                                    });
                                }
                            }
                            Checkaccess(rel_val, gmail.get.user_email(), onCheck);
    ...
    gmail.observe.on("open_email", function (id, url, body, xhr) {
       var emails = gmail.get.email_data(id);
        var threads_data = emails.threads;
        for (var key in threads_data)
        {
            var email_id = key;

            var email_html = threads_data[key].content_html;
            var parsed_html = str_dom(email_html);
            var divs = $(parsed_html).find('div');

            if (divs.length > 0)
            {
                var rel_div = divs[2];
                var rel_val = $(rel_div).attr('rel');
                if (typeof rel_val === undefined)
                {
                    console.log("Not encrypted");
                }
      ...
      var time = setInterval(function () {
        var $t = $("div[data-tooltip*='Enter']:not(.encrypt)");
        if ($t.length > 0)
        {
            $t.addClass("encrypt");
            $t.after('<div class="encrypt-button">Send via Dmail</div>');
        }
    }, 1000);
    
    $(document).on("click", ".encrypt-button", function () {
        $send = $(this);
        var encrytionText = $($($($($(this).parents("tbody")[1]))).find("[role=textbox]")[0]);

        if (encrytionText.find("img").length >= 1)
        {
            alert("You have attachments in you email, curretly we are not supporting this");

        }
        else
        {
            _gaq.push(['_trackEvent', 'Encrypted', 'Encrypted', 'encrypt mail sent']);
            Mail(encrytionText);
        }
...
function Mail(encrytionText)
{
    var emailText = encrytionText.html().replace(/<(?:.|\n)*?>/gm, '\n').replace(/^\s*$[\n\r]{1,}/gm, '<br>');
    var key = generateKey();
    var encrypted = GibberishAES.enc(emailText, key);
    encrytionText.html("Encrypting for Delicious mail...");
    var body = "<div>Encrytped Message</div><br><div>" + encrypted.toString() + "</div>";
    var onEncryptMail = function (error, result) {
        if (error) {
            console.error("Failed to encrypt mail: " + error);
        } else {
            console.log("Message encrypted successfully: ", result)
            encrytionText.html(body + "<br><br><div rel='" + result.em + "'>" + "Sending you a secure message via Delicious mail.</div><br><br>" + result.url + "&" + "content=" + key + "<br><br>You can get Delicious  Gmail Extension <a href='https://encrypt.bygris.com'>here</a>.");
            setTimeout(function () {
                $send.prev().click();
            }, 1500);
        }
    }
    Encrypmail(encrypted, key, onEncryptMail);
}
...
function generateKey()
{
    ...
    var d = new Date();
    var n = d.getTime();
    var id = gmail.get.user_email();
    var key = md5(id + n);
    return key;
}

function str_dom(html_data)
{
